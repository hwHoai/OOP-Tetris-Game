...
public class LPiece extends Piece {
    public LPiece(){
        super(new Block[]{
                new Block((int) (Math.floor(PlayView.COLS / 2 ) * Block.width), 0, "#00FF00"),
                new Block((int) (Math.floor(PlayView.COLS / 2 ) * Block.width), 20, "#00FF00"),
                new Block((int) (Math.floor(PlayView.COLS / 2 ) * Block.width), 40, "#00FF00"),
                new Block((int) (Math.floor(PlayView.COLS / 2 ) * Block.width) + Block.width, 40, "#00FF00"),
        });
    ...
    public void rotate() {
        switch (getForm()) {
            ...
            case 4:{
                if(!isLeftCollided() && !isRightCollided()) {
                    int nexPosOfBlock0 = PlayView.board[(int) (getBlocks()[0].getTranslateX() - Block.width*2) / Block.width][(int) (getBlocks()[0].getTranslateY() - (Block.height*1)) / Block.height];
                    int nexPosOfBlock1 = PlayView.board[(int) (getBlocks()[1].getTranslateX() - (Block.width * 1)) / Block.width][(int) (getBlocks()[1].getTranslateY()) / Block.height];
                    ...
                    int nexPosOfBlock3 = PlayView.board[(int) (getBlocks()[3].getTranslateX() + (Block.width * 1)) / Block.width][(int) (getBlocks()[3].getTranslateY()) / Block.height];
                    if (nexPosOfBlock0 == 0 && nexPosOfBlock0 == nexPosOfBlock1 && nexPosOfBlock1 == nexPosOfBlock2 && nexPosOfBlock2 == nexPosOfBlock3) {
                        getBlocks()[0].moveLeft();
                        getBlocks()[0].moveLeft();
                        getBlocks()[0].moveUp();
